...
ANALYSES = (
    ...
    ("family", ROOT / "scripts/r4_family_analysis.py",
     ROOT / "results/r4_final/analysis/family_comparisons.csv"),
)
...
sys.argv = ['sweep', '--step', 'analysis', '--out', {out!r},
            '--paper-dir', {paper!r}] + (
    ['--skip-reconcile'] if 'family' in {out!r} else [])
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="r4_margin_") as td:
        for aname, script, base_csv in ANALYSES:
            ...
            for tag, k in SWEEPS:
                ...
                alt_csv = out / ("family_comparisons.csv" if aname == "family"
                                 else "equivalence.csv")
                ...
                if aname == "family":                # verdict-only layout
                    jk = ["scope_type", "scope", "family"]
                    m = base[jk + ["verdict"]].merge(
                        alt[jk + ["verdict"]], on=jk,
                        suffixes=("_base", "_swept"))
                    m["in_equivalence_set_base"] = (m.verdict_base
                                                    == "equivalent").astype(int)
                    m["in_equivalence_set_swept"] = (m.verdict_swept
                                                     == "equivalent").astype(int)
                else:
    ...
    nfam = counts[("family", "half")][0]
    cfam_h = counts[("family", "half")][1]
    cfam_d = counts[("family", "double")][1]
    ...
    macros = [
        ...
        "\\newcommand{\\mgsFamMemberships}{%d}" % nfam,
        "\\newcommand{\\mgsFamHalfLeave}{%d}" % cfam_h,
        "\\newcommand{\\mgsFamDoubleJoin}{%d}" % cfam_d,
